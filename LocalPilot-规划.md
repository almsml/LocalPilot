...
> **一句话**：把你吃灰的旧电脑、旧手机变成你的「私人 AWS 集群」——提交任务，自动调度，并行计算。
...
> 「我有台旧 MacBook 和两台旧笔记本吃灰很久了，而主力机跑重任务时经常 CPU 满载。我就想：为什么不能像 AWS 那样，把我手边的设备组成一个计算集群？于是做了 LocalPilot——一个能在 5 分钟内把多台设备变成私人计算网格的系统。」

### 1.2 一句话叙事（面试官追问"有什么特别的"）

> 「它不是一个玩具 Demo。它解决了三个真问题：**异构设备的自动发现与组网**、**基于设备能力的自适应任务调度**、**节点离线时的任务自动迁移**。而且所有通信走 mTLS，任务在沙箱里跑——你从浏览器里把一台旧设备当成计算节点用，体验却像在用云服务。」
...
```
                       ...
                       ▼                 ▼                 ▼
              ┌──────────────┐ ┌──────────────┐ ┌──────────────┐
              │ ⚙️ Agent (Go)  │ │ ⚙️ Agent (Go)  │ │ ⚙️ Agent (Go)  │
              │   MacBook     │ │   Mac Mini     │ │   旧笔记本      │
              │               │ │               │ │               │
              │ • 能力上报     │ │ • 能力上报     │ │ • 能力上报     │
              │ • 任务执行     │ │ • 任务执行     │ │ • 任务执行     │
              │ • 沙箱隔离     │ │ • 沙箱隔离     │ │ • 沙箱隔离     │
...
| 组件 | 语言 | 核心理由 |
|------|------|---------|
| **Agent（工作节点）** | **Go** | ① 与 Controller 统一语言栈，降低认知负担 ② 交叉编译简单（GOOS/GOARCH），旧设备也能跑 ③ goroutine 天然适合心跳和 gRPC 并发 ④ 单二进制部署，零运维 |
...
> **为什么 Agent 和 Controller 都用 Go？不做异构语言设计吗？**

回答模板：

> 「最初确实计划 Agent 用 Rust、Controller 用 Go，追求异构语言设计。但在 Phase 0 开发中，Rust 的学习曲线严重拖慢了项目进度——所有权/借用/生命周期概念消化耗时，tonic 和 sysinfo 的文档不如 Go 生态直观，调试编译错误的时间远超写业务逻辑。决定 Agent 也切到 Go 后，两边的 proto 生成代码可以直接共享（通过 go.mod replace），交叉编译同样简单（GOOS=linux GOARCH=arm64），而且 goroutine 模型天然适合心跳循环和 gRPC 并发。统一语言栈降低了整体认知负担，让我能聚焦在分布式系统的核心问题上——调度、容错、服务发现——而不是和编译器搏斗。这个决策的核心逻辑是**用务实换取进度**。」
...
require (
    ...
    github.com/shirou/gopsutil/v4  // 系统信息采集（CPU/内存/温度，跨平台）
    github.com/hashicorp/mdns      // mDNS 服务发现
    github.com/docker/docker       // Docker SDK for Go（容器管理）
)
```
...
```
□ 手动在两台设备间建立 gRPC 连接（都是 Go）
□ 验证 mDNS 能发现局域网内的设备
□ Go Agent 读取系统信息并上报给 Go Controller
□ 在 Controller 收到上报后，Web 页面显示设备信息
```

**产出**：一个 200 行的原型，证明 Go↔Go gRPC 通信 + mDNS 发现的链路是通的。

**风险点**：
- mDNS 在 Windows 上的行为可能与 macOS/Linux 不同
- Go gRPC 的 proto 生成路径要统一
...
□ 系统信息采集 (gopsutil: CPU %, 内存使用, 磁盘, 温度)
□ 优雅退出 (SIGTERM → Deregister)

要点：
- 心跳不要阻塞主循环，用 goroutine
...
□ 执行命令 (os/exec + 管道捕获 stdout/stderr)
...
```
沙箱（多层降级策略）：
□ 检测 Docker 可用 → 使用 Docker SDK for Go 创建容器执行
...
### Demo 1：并行视频转码（主力展示）
```
准备: 一段 4K/10 分钟的测试视频, 3 台设备（MacBook + Mac Mini + 旧笔记本）
...
### Demo 3：异构能力匹配（展示调度器）
```
准备: 一个需要 GPU 的 AI 推理任务, 一个有 GPU 的 MacBook, 两台无 GPU 的旧笔记本

操作:
1. 提交 AI 推理任务
2. 调度器自动排除了两台旧笔记本（无 GPU）
3. 任务自动匹配到有 GPU 的 MacBook

台词:
> 「调度器不是随机选设备——它理解每台设备的能力，确保 GPU 任务不会
> 被发到旧笔记本上。这是一个能理解异构硬件的调度系统。」
```
...
### 6.1 为什么 Agent 和 Controller 都用 Go？

> 「最初规划是 Agent 用 Rust、Controller 用 Go，追求异构语言设计。但在 Phase 0 实际开发中，Rust 的学习曲线远超预期——所有权和生命周期概念消耗大量时间，tonic 的 API 不如 Go gRPC 直观，调试编译错误的时间远超写业务逻辑。决定统一到 Go 后，两边共享同一份 proto 生成的代码、交叉编译同样简单、goroutine 天然匹配心跳和并发的场景。这个决策的核心是**务实优于炫技**——我宁可花时间打磨分布式系统的调度和容错，而不是和编译器搏斗。这个教训本身就很有价值：在真实项目中，进度和认知负担是比语言纯度更重要的约束。」
...
> 「K8s 面对的是数据中心里的同构节点，调度考虑 CPU/内存/亲和性。LocalPilot 面对的是**极度异构**的个人设备——一台 MacBook 有 GPU，旧笔记本是 ARM，Windows 不支持 Docker——调度器必须理解这些异构能力。K8s 的调度器是通用解，LocalPilot 的调度器是**面向个人异构环境的专用解**，更轻量，也更容易理解。」
...
| Go gRPC TLS 配置复杂 | 中 | 中 | Phase 1 先用明文通信，Phase 4 再加 mTLS |
| gRPC 在弱网下表现差 | 低 | 高 | gRPC 本身有 HTTP/2 keepalive；额外增加应用层心跳超时检测 |
| 旧设备上 Docker 性能差 | 中 | 低 | 提供无 Docker 的进程级执行备选方案（Linux cgroups + namespaces） |
...
│   ├── cmd/
│   │   └── agent/
│   │       └── main.go             # 入口：启动 mDNS + gRPC server
│   └── internal/
│       ├── config/                 # 配置管理
│       ├── discovery/              # mDNS 广播 & 服务注册
│       ├── transport/              # gRPC client (连接 Controller)
│       ├── executor/               # 任务执行器 (命令执行 + 日志捕获)
│       ├── sandbox/                # 沙箱抽象层 (Docker / 进程隔离)
│       ├── monitor/                # 系统资源监控 (gopsutil)
│       └── heartbeat/              # 心跳循环
│
...
Phase 1 (周 3-6):   ═══════════███░░░░░░░░░░  设备底座, 60% Go + 30% React + 10% 其他
Phase 2 (周 7-10):  ════════════════███░░░░░  任务执行, 50% Go + 30% React + 20% 其他
Phase 3 (周 11-14): ════════════════════███░  智能调度, 70% Go + 30% React
Phase 4 (周 15-17): ═══════════════════════██  容错韧性, 70% Go + 30% React
